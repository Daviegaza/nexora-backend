...
export async function registerProductRoutes(app: FastifyInstance) {
  ...
  app.post(
    '/',
    { preHandler: [requireCap('products.create')], schema: { body: ProductInput } },
    async (req) => {
      const me = (req as unknown as { me: { workspaceId: string } }).me;
      return prisma.product.create({
        data: { ...(req.body as z.infer<typeof ProductInput>), workspaceId: me.workspaceId },
      });
    },
  );
  ...
  app.put(
    '/:id',
    { preHandler: [requireCap('products.update')], schema: { body: ProductInput.partial() } },
    async (req, reply) => {
      const me = (req as unknown as { me: { workspaceId: string } }).me;
