...
import { requireAuth, requireCap, hashPassword } from '../../lib/auth.js';
...
export async function registerUserRoutes(app: FastifyInstance) {
  app.get('/', { preHandler: [requireCap('users.read')] }, async (req) => {
    ...
    const users = await prisma.user.findMany({
      ...
      select: {
        ...
        branchId: true, status: true, lastLoginAt: true, createdAt: true,
        permissionsAdd: true, permissionsRemove: true, customRoleId: true,
      },
  ...
  app.post('/invite', {
    ...
    const me = (req as unknown as { me: { workspaceId: string } }).me;
    const b = req.body as { name: string; email: string; phone?: string; role: string; branchId?: string; tempPassword: string };
    const passwordHash = await hashPassword(b.tempPassword);
    const user = await prisma.user.create({
      data: {
        workspaceId: me.workspaceId, name: b.name, email: b.email, phone: b.phone,
        role: b.role as never, branchId: b.branchId, passwordHash,
      },
  ...
  app.patch('/:id/grant', {
    ...
    if (u.workspaceId !== me.workspaceId) throw new Error('Cross-workspace mutation blocked');
  ...
  app.patch('/:id/deactivate', { preHandler: [requireCap('users.deactivate')] }, async (req) => {
    const { id } = req.params as { id: string };
